...
const carImages = {
  'Mercedes-Benz S-Class': 'https://images.pexels.com/photos/120049/pexels-photo-120049.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=2',
...
// Replace the images array inside each vehicle object
for (const [modelKey, url] of Object.entries(carImages)) {
  const regex = new RegExp(`(model:\\s*'${modelKey}'[\\s\\S]*?images:\\s*\\[)\\s*'[^']*'\\s*(\\])`, 'g');
